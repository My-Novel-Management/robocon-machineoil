...
def sc_aboutcase(w: World):
    ...
    return w.scene("事件について",
            ...
            emil.do("白い壁はどの部屋でも変わらないらしい",
                ...
                "ジャケットからメモ帳を取り出したくなっていた"),
            paul.talk("それで$nielがいる部屋に入った時には誰もいなかった、そう言いたいんだな？"),
            paul.do("声音こそ穏やかだが口調は完全に尋問のそれだった", "&"),
            _.look("ロボットだと言っても通じそうなぴったりと九対一で固められた金髪の前髪に、ほとんど変化のない目元が刑事の冷徹さを感じさせる"),
            ...
            emil.talk("だからそれは$meには分かりません",
                "記録が間違っている可能性を考えた方が良いんじゃないですか？"),
            ...
            paul.talk("部屋に入って$nielがいないと気づいてから$uruを端末で呼ぶまでの間、何をしていた？"),
            emil.do("反論や意見を口にする間もなく矢継ぎ早に質問が浴びせられる"),
            emil.talk("彼を探していました", "キッチンやベッドルームにいるかも知れないと考えて"),
            paul.talk("それを証明できるものは？"),
            emil.talk("知りませんよ", "監視カメラはないんですか？"),
            paul.talk("ある", "だが答えられない"),
            emil.do("肝心なことについては何も教えてもらえない",
                "いつもこうだ",
                "事件に関する情報について上手く聞き出すのは取材技術だと先輩に教わったが、",
                "最初から教える気のない相手からはどうやっても引き出すことはできないと諦めることも大事だった"),
            ...
            paul.talk("こちらの方に泊まっていただきます"),
            emil.talk("ここに、ですか？"),
            paul.talk("そうなりますね",
                "快適とは言い難いでしょうが部屋には警備用ロボットも立たせますし、外部とのやり取りは許可できませんが、",
                "端末を利用して仕事をしてもらって構いません",
                "ただし本件についての記事を書かれては困りますが"),
            emil.do("そう答えて立ち上がると、そこで初めて$paulは笑みを見せた",
                    "冗談のつもりだったのだろうか", "精神的に疲弊していた$Sにはとても判別がつかなかった"),
            emil.talk("あの"),
            emil.do("部屋を出ていこうとした彼を呼び止める"),
            emil.talk("$meの使う部屋はここですか？"),
            paul.talk("いえ", "$uruという社員に手配してもらっているから、後で彼に迎えに来させよう",
                    "それまではここから動かないでいてもらいたい", "いいかな？"),
            emil.do("イエス以外の選択肢がないことを分かっていてわざわざ尋ねるのは相手が精神的にマウントを取る為の手法だと聞いている",
                    "それでも仕方なく「はい」と答えて$Sは、完全に事件について記事する機会を逸したことを理解した"),
            paul.talk("ああ、そうだ",
                    "君の連絡先を貰おう", "とりあえず常時応答ができるようにしておいてくれ",
                "端末は持っているだろう？"),
            emil.talk("はい"),
            emil.do("力なくそう答え、腕時計型の端末を彼が差し出した携帯型端末に向けた"),
...
def sc_research(w: World):
    ...
    return w.scene("調査",
            ...
            uru.be(),
            adam.be(),
            emil.do("簡易ベッドの上に座り、その弾力が普段$Sが使っている最低ランクのホテルよりずっと良いことに驚く",
                "$Sが事件に巻き込まれなければ誰も泊まる予定などなかっただろうに、",
                "シーツには塵一つ付着していない"),
            ...
            uru.talk("人間らしさを担保する為に時には無駄なことをするのも良いですよ",
                "例えば$meはピアノを弾きます"),
            emil.do("$uruは笑みを見せて宙で鍵盤を弾く真似をしたが、$Sにはそれを本気で言っているのかどうか、やはり理解できなかった"),
            ...
            uru.talk("基本的な世話はこの$full_adamに頼めばいい",
                "$nielほどじゃないが技術的な質問にもある程度答えられるし、家事能力は$meよりも遥かに高い",
                "将来的には一家に一台、コンシェルジュのような存在にすることも計画にある",
                "ただまだまだ人間とロボットの共存を危険視する連中は多いがね"),
            adam.look("頭髪のないつるりとした外観は間近に立たれると威圧感がある",
                "多くのロボットが性別を意識しなくて済むように中性的、かつ体毛のないデザインをしていたが、",
                "$w_cias社のものは肌をべったりとしたカラーリングにしている所為か、",
                "必要以上に非人間性を感じさせた"),
            emil.talk("あの、この個体は特別なんですか？"),
            uru.talk("いえ", "$w_droidには個性は与えていません", "何故ですか？"),
            emil.talk("色が"),
            uru.talk("それはおそらく修繕後の塗装時に同じ色がなかったのでしょう", "よくあることです"),
            adam.look("彼の右肩には$w_cias社のマークが入っていたが、それが上から塗られたオレンジによって消えかかっているようにも見えた"),
            ...
            uru.go("#部屋を出ていく"),
            emil.talk("よろしく……えー、なんてお名前？"),
            adam.talk("$meは$full_adamです"),
            emil.hear("声は男性の、やや高いものを採用していた", "&"),
            emil.think("しかし型番で呼ぶという訳にもいかないと考え、$Sは「$adamではどうかしら」と提案した"),
            ...
            adam.talk("はい", "それはゲストの$emilia様でも許可されています"),
            ...
            adam.talk("デリバリーを頼みますか？"),
            emil.talk("やっぱりそうなるか……あ、ねえ、ちょっと待って",
                    "$nielさんの部屋って$meが入ることはできるかしら？"),
